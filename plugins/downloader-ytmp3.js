...
function tokenizer(nyxz, ndbz) {
    if (eval(Buffer.from(nyxz.t[0], 'base64').toString()) == nyxz.t[1]) {
        for (var c = 0; c < Buffer.from(nyxz[0], 'base64').toString().split(nyxz.f[5]).length; c++) ndbz += (0 < nyxz.f[4] ? nyxz[1].split("").reverse().join("") : nyxz[1])[Buffer.from(nyxz[0], 'base64').toString().split(nyxz.f[5])[c] - nyxz.f[3]];
...
async function ytdl(url) {
    ...
    try {
        ...
        const tokenJson = JSON.parse(Buffer.from(webb.data?.match(/atob\('(.*?)'\)/)?.[1], 'base64').toString().match(/var gC = ({[\s\S]*?});/)?.[1]);
        const token = Buffer.from(tokenJson[2] + "-" + tokenizer(tokenJson, tokenJson.f[6])).toString('base64');
